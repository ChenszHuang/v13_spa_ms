...
class AccountMove(models.Model):
    ...
    def _compute_payment_ids(self):
        for move in self:
            ...
            matched_lines = receivable_lines.mapped('matched_debit_ids.debit_move_id') | \
                            receivable_lines.mapped('matched_credit_ids.credit_move_id')

            payments = matched_lines.mapped('payment_id').filtered(lambda p: p)
    ...
    def _compute_payments_widget_to_reconcile_info(self):
        for move in self:
            ...
            lines = lines.filtered(lambda l: not l.payment_id or l.payment_id.state != 'cancelled')
